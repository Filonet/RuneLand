...
class PlayerDataHelper {
    ...
    public function addMoney(string $nickname, int $money): void{
        ...
        Loader::$mThread->pushQueryPacket('INSERT INTO `stats` (`nickname`, `money`, `runes`, `kills`, `deaths`, `gameTime`) VALUES("' . $nickname . '", ' . $money . ', 0, 0, 0, 0) ON DUPLICATE KEY UPDATE `money` = `money` + ' . $money . ';');
    }
    ...
    public function addRunes(string $nickname, int $runes): void{
        ...
        Loader::$mThread->pushQueryPacket('INSERT INTO `stats` (`nickname`, `money`, `runes`, `kills`, `deaths`, `gameTime`) VALUES("' . $nickname . '", 0, ' . $runes . ', 0, 0, 0) ON DUPLICATE KEY UPDATE `runes` = `runes` + ' . $runes . ';');
    }
