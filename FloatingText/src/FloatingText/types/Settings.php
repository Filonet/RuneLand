<?php

declare(strict_types=1);

namespace FloatingText\types;

class Settings {
    public function __construct(){
        //NOOP
    }

    /*
     * x y z text
     */
    public const array FLOATING_TEXTS = [

        [207.5, 126, 197.5,   "%floating.text.useful.commands.title%"],
        [207.5, 125.7, 197.5, "%floating.text.useful.commands.rtp%"],
        [207.5, 125.4, 197.5, "%floating.text.useful.commands.wextend%"],
        [207.5, 125.1, 197.5, "%floating.text.useful.commands.ps%"],
        [207.5, 124.8, 197.5, "%floating.text.useful.commands.ah%"],
        [207.5, 124.5, 197.5, "%floating.text.useful.commands.gps%"],
        [207.5, 124.2, 197.5, "%floating.text.useful.commands.help%"],
        [207.5, 123.9, 197.5, "%floating.text.useful.commands.clan%"],
        [207.5, 123.6, 197.5, "%floating.text.useful.commands.board%"],
        [207.5, 123.3, 197.5, "%floating.text.useful.commands.itemtoggle%"],
        [207.5, 123, 197.5,   "%floating.text.useful.commands.shop%"],
        [207.5, 122.7, 197.5, "%floating.text.useful.commands.buyer%"],
        [207.5, 122.4, 197.5, "%floating.text.useful.commands.exchange%"],
        [207.5, 122.1, 197.5, "%floating.text.useful.commands.battlepass%"],
        [207.5, 121.8, 197.5, "%floating.text.useful.commands.donate%"],
        [207.5, 121.5, 197.5, "%floating.text.useful.commands.links%"],

        /*
            [90, 98.6, 90, "%floating.text.auto.mine%"],
            [90, 98.3, 90, "%floating.text.auto.mine.end.nether%"],

            [80, 98.6, 80, "%floating.text.auto.mine%"],
            [80, 98.3, 80, "%floating.text.auto.mine.normal%"],
         */

        [193.5, 124.3, 197.5,   "%floating.text.list.title%"],
        [193.5, 124.0, 197.5, "%floating.text.list.group.mouth.hero%"],
        [193.5, 123.7, 197.5, "%floating.text.list.group.mouth.hunter%"],
        [193.5, 123.4, 197.5, "%floating.text.list.group.mouth.ranger%"],
        [193.5, 123.1, 197.5, "%floating.text.list.group.mouth.elemental%"],
        [193.5, 122.8, 197.5, "%floating.text.list.group.mouth.phantom%"],
        [193.5, 122.5, 197.5, "%floating.text.list.group.mouth.arcana%"],
        [193.5, 122.2, 197.5, "%floating.text.list.group.mouth.titan%"],
        [193.5, 121.9, 197.5, "%floating.text.list.group.mouth.elder%"],
        [193.5, 121.6, 197.5, "%floating.text.list.dc.donate%"],
        [193.5, 121.3, 197.5,   "%floating.text.list.dc.money%"],
        [193.5, 121, 197.5, "%floating.text.list.buy.site%"],

        [324.5, 104.5, 83.5, "%floating.text.hero1%"],
        //[324.5, 104.2, 83.5, ""],
        [324.5, 103.9, 83.5, "%floating.text.hero2%"],
        [324.5, 103.6, 83.5, "%floating.text.hero3%"],
        [324.5, 103.3, 83.5, "%floating.text.hero4%"],
        //[324.5, 103, 83.5, ""],
        [324.5, 102.7, 83.5, "%floating.text.hero5%"],
        [324.5, 102.4, 83.5, "%floating.text.hero6%"],
        [324.5, 102.1, 83.5, "%floating.text.hero7%"],
        [324.5, 101.8, 83.5, "%floating.text.hero8%"],
        //[324.5, 101.5, 83.5, ""],
        [324.5, 101.2, 83.5, "%floating.text.hero9%"],
    ];
}